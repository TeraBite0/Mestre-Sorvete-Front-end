...
const Estoque = () => {
  ...
  //   const token = sessionStorage.getItem('token');
  //     try {
  //        await axios.get('http://54.80.52.5:80/api/relatorio', {
  //         headers: {
  //           Authorization: `Bearer ${token}`
  //         }
  //       });
  ...
  const baixarRelatorioExcel = async () => {
    ...
    try {
      const response = await axios.get('http://54.80.52.5:80/api/relatorio', {
        headers: {
          Authorization: `Bearer ${token}`
        },
  ...
  const abrirModalAdicionarLote = async () => {
    const token = sessionStorage.getItem('token');
    try {
      const response = await axios.get('http://54.80.52.5:80/api/fornecedores', {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
  ...
  const handleSubmitLote = async (formData) => {
    ...
    try {
      await axios.post('http://54.80.52.5:80/api/lotes', jsonParaCriarLote, {
        headers: {
          'Authorization': `Bearer ${token}`,
