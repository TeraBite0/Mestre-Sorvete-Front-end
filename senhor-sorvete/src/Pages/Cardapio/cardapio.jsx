...
const Cardapio = () => {
    ...
    const fetchProdutos = async () => {
      ...
      try {
        const response = await axios.get(
          "http://10.0.0.25:8080/produtos/ativos"
        );
  ...
  // Basicamente essa função uso os dados dos produtos buscados anteriormente
    ...
    //   try {
    //     const response = await axios.get("http://10.0.0.25:8080/produtos/populares");
  ...
  const sendNotification = async () => {
    ...
    const notifications = cartItems.map(async (item) => {
      try {
        const response = await axios.post("http://10.0.0.25:8080/notificacoes", {
          email,
          idProduto: item.id,
        });
